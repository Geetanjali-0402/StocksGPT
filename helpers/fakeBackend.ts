...
import { graphJSON } from "../data/graphJSON";
...
const fakeBackend = () => {
  ...
  // mock.onPost(url.GET_STOCK_CHAT_TITLES).reply(config => {

  //   const userId  = JSON.parse(config.data);
  //   // In real back user ID needs to be extracted
  //   // Prepares response with chat details
  //   const response = stockChats.map(chat => ({
  //     chatId: chat.chatId,
  ...
  // // Mock for adding new chat
  // mock.onPost(url.ADD_NEW_CHAT).reply(config => {
    
  //   const { userId, message } = JSON.parse(config.data);
    
  //   // Generate next chatId. Assuming chatId is numeric for simplicity
  //     // Find the maximum chatId in the existing data
  //   const maxChatId = Math.max(...stockChats.map(chat => chat.chatId), 0);
  //   const newChatId = maxChatId + 1 // Generate next chatId
    
  //   // Assuming the structure of a message in your chat messages array
  //   const newMessage = {
  //     mId: 1, // Starting message ID for the new chat
  //     text: message,
  //     time: new Date().toISOString(),
  //     sender: userId, // Assuming the sender's name or ID is directly mapped from userID
  //   };

  //   const firstThreeWords = message.split(' ').slice(0, 3).join(' ');

  //   // Create a new chat with the provided message as the first message
  //   const newChat = {
  //     chatId: newChatId,
  //     chatTitle: firstThreeWords, // Assuming a default title, adjust as needed
  //     lastUpdateDate: new Date(),
  //     messages: [newMessage], // Add the new message to this chat
  //   };

  //   // Add the new chat to the array of stock chat
  //   stockChats.push(newChat);

  //   const lastMessageText = newMessage.text;
  //   const botResponseText = 'Hey!! I am your Stocks Buddy but am not live yet 😊' + 
  //                   '\nBut i did hear your request >> ' + lastMessageText ;
  //   const plotlyJson = graphJSON;
  
  //   const BotResponseMessage = {
  //     mId: 2, // Starting message ID for the new chat
  //     text: botResponseText,
  //     time: new Date().toISOString(),
  //     sender: "BOT", // Assuming the sender's name or ID is directly mapped from userID
  //     plotlyJson: plotlyJson
  //   };

  //   newChat.messages.push(BotResponseMessage);


  //   onChangeStockChats(stockChats);

  //   // Return the new chat in the response
  //   return [200, newChat];
  // });


  // // Mock for deleting a stock chat
  // mock.onPost(url.DELETE_STOCK_CHAT).reply((config) => {
  //   const { userId, chatId } = JSON.parse(config.data);
  
  //   // Check for missing parameters
  //   if (!userId || !chatId) {
  //     return [400, "Missing required parameters (userId or chatId)"];
  //   }
  
  //   // Find the chat index based on chatId
  //   const index = stockChats.findIndex((chat) => chat.chatId === chatId);
    
  //   if (index === -1) {
  //     return [404, "Chat not found"];
  //   }
  
  //   // Perform deletion and update based on chat index
  //   stockChats.splice(index, 1);
  //   onChangeStockChats(stockChats);
  
  //   return [200, { message: "Chat deleted successfully" }];
  // });


  // // Mock for getting stock chat messages
  // mock.onPost(url.GET_STOCK_CHAT_MESSAGES).reply(config => {
  ...
  //   const chat = stockChats.find(c => c.chatId === chatId);
  //   if (!chat) {
  //     return [404, "Chat not found"];
  //   }
    
  //   return [200, chat.messages];
  // });

  // // mock for sending a chat..
  // mock.onPost(url.SEND_USER_MESSAGE).reply(config => {

  //   if (!config.data) { 
  //     return [400, "No message data provided"];
  //   }
  
  //   const { userId, chatId, message } = JSON.parse(config.data);

  //   // Check for missing parameters
  //   if (!userId || !chatId || !message) {
  //     return [400, "Missing required parameters (userId or chatId or user message)"];
  //   }
  ...
  //   const maxMessageId = Math.max(...chat.messages.map(chat => chat.mId), 0);
  //   const newMessageId = maxMessageId + 1 // Generate next MessageID

  //   const newUserMessage = {
  //     mId: newMessageId, // New message ID for the new message
  //     text: message,
  //     time: new Date().toISOString(),
  //     sender: userId, // Assuming the sender's name or ID is directly mapped from userID
  //   };
  ...
  //   const lastMessageText = newUserMessage.text;
  //   const botResponseText = 'Hey!! I am your Stocks Buddy but am not live yet 😊' + 
  //                   '\nBut i did hear your request >> ' + lastMessageText 
  
  //   const BotResponseMessage = {
  //     mId: maxMessageId + 2, // Starting message ID for the new chat
  //     text: botResponseText,
  //     time: new Date().toISOString(),
  //     sender: "BOT", // Assuming the sender's name or ID is directly mapped from userID
  //   };

  //   chat.messages.push(BotResponseMessage);
  //   onChangeStockChats(stockChats);
  
  //   return [200, BotResponseMessage];
  // });
  

  // mock.onPost(url.RECEIVE_BOT_MESSAGE).reply(config => {
  //   if (!config.data) {
  //     return [400, "No request data provided"];
  //   }
  ...
  //   const BotResponseMessage = {
  //     mId: chat.messages.length, // Starting message ID for the new chat
  //     text: botResponseText,
  //     time: new Date().toISOString(),
  //     sender: "BOT", // Assuming the sender's name or ID is directly mapped from userID
  //   };
